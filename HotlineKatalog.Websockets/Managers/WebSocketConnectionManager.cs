...
namespace HotlineKatalog.WebSockets.Managers
{
    public class WebSocketConnectionManager<ConnectedData> where ConnectedData : class, IWSItem
    {
        ...
        public List<WSItem<ConnectedData>> GetAll()
        {
        ...
        {
            var sockets = Sockets.Where(p => p.Data.UserId == userId).Select(s => s.Socket).ToList();

            return sockets;
        }

        public List<WebSocket> GetSocketsByUserIdSocket(int userId, WebSocket socket)
        {
            var sockets = Sockets.Where(p => p.Socket != socket && p.Data.UserId == userId).Select(s => s.Socket).ToList();
